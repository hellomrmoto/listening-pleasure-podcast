...
export default function Pitch() {
  ...
  return (
    ...
    >
      ...
      {/* Marquee Header */}
      <header role="banner" className="w-full overflow-hidden bg-white text-black py-3 border-b border-white/10 z-10 relative flex whitespace-nowrap" aria-hidden="true">
      ...
      <main id="main-content" role="main" className="flex-1 relative z-10 max-w-7xl mx-auto w-full px-6 sm:px-8 md:px-12 lg:px-16 py-12 flex flex-col md:flex-row gap-16 items-center justify-center">
        
        {/* Left Column: Copy & Socials */}
        <section aria-labelledby="pitch-heading" className="w-full md:w-1/2 flex flex-col">
          ...
          <h1 id="pitch-heading" className="font-display text-5xl sm:text-6xl md:text-7xl lg:text-8xl leading-[0.9] tracking-tight uppercase mb-6">
            Drop The <br/>
          ...
          <aside className="mt-auto">
            <p className="font-mono text-xs tracking-[0.2em] text-neutral-500 uppercase mb-4" id="share-heading">Share to get upvotes</p>
            <div className="flex gap-4" aria-labelledby="share-heading">
              <button onClick={handleNativeShare} className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-300 group" aria-label="Share via native sharing">
                <Share2 className="w-5 h-5 group-hover:scale-110 transition-transform" aria-hidden="true" />
              </button>
              <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(shareTitle)}&url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-[#1DA1F2] hover:border-[#1DA1F2] transition-all duration-300 group" aria-label="Share on Twitter">
                <Twitter className="w-5 h-5 group-hover:scale-110 transition-transform" aria-hidden="true" />
              </a>
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-[#4267B2] hover:border-[#4267B2] transition-all duration-300 group" aria-label="Share on Facebook">
                <Facebook className="w-5 h-5 group-hover:scale-110 transition-transform" aria-hidden="true" />
              </a>
              <a href={`mailto:?subject=${encodeURIComponent("Pitch The Next Episode")}&body=${encodeURIComponent(shareTitle + " " + shareUrl)}`} className="w-12 h-12 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black transition-all duration-300 group" aria-label="Share via Email">
        ...
        {/* Right Column: Form */}
        <section aria-label="Pitch Submission Form" className="w-full md:w-1/2 max-w-md">
            ...
            <AnimatePresence mode="wait">
              {!isSuccess ? (
                ...
                >
                  ...
                  {errorMsg && (
                    <div id="form-error" className="text-red-400 text-sm font-mono bg-red-400/10 p-3 rounded-lg border border-red-400/20" role="alert">
                      {errorMsg}
                    </div>
                  )}
